	...
	d(new FloatingDockContainerPrivate(this))
{
	...
	if ((DockManager->testConfigFlag(CDockManager::FloatingContainerForceQWidgetTitleBar)
		|| DockManager->testConfigFlag(CDockManager::FloatingContainerForceQWidgetCustomStyledTitleBar))
		&& !native_window
		&& !DockManager->testConfigFlag(CDockManager::FloatingShadowEnabled))
	{
...
void CFloatingDockContainer::resizeEvent(QResizeEvent *event)
{
	...
	if (!isMaximized())
	{
		...
		Qt::Edges e;
		if (p.x() <= windowHandle()->geometry().topLeft().x())
		{
			e | Qt::Edge::LeftEdge;
		}
		if (p.y() <= windowHandle()->geometry().topRight().y())
		{
			e | Qt::Edge::TopEdge;
		}
		if (p.x() >= windowHandle()->geometry().topRight().x())
		{
			e | Qt::Edge::RightEdge;
		}
		if (p.y() >= windowHandle()->geometry().bottomRight().y())
		{
