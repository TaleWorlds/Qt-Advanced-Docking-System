...
private Q_SLOTS:
    ...
    /*
     * Update the auto hide button checked state based on if it's contained in an
     * auto hide container or not
     */
...
protected:
     ...
     */
    void insertDockWidget(int index, CDockWidget* DockWidget,
                          bool Activate = true);
...
public:
    ...
    /**
     * Returns the auto hide dock container widget this dock area widget belongs
     * to or 0 if there is no
     */
     ...
     */
    static bool restoreState(CDockingStateReader& Stream,
                             CDockAreaWidget*& CreatedWidget, bool Testing,
                             CDockContainerWidget* ParentContainer);
    ...
    CDockWidget::DockWidgetFeatures features(
        eBitwiseOperator Mode = BitwiseAnd) const;
    ...
    /**
     * Configures the areas of this particular dock area that are allowed for
     * docking
     */
    ...
    /**
     * Returns true if the area has a single dock widget and contains the central
     * widget of it's manager.
     */
