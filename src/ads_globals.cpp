...
//============================================================================
void xcb_update_prop(bool set, WId window, const char* type, const char* prop,
                     const char* prop2)
{
...
//============================================================================
xcb_get_property_reply_t* _xcb_get_props(WId window, const char* type,
                                         unsigned int atom_type)
{
    ...
    xcb_get_property_cookie_t request = xcb_get_property_unchecked(
        connection, 0, window, type_atom, atom_type, 0, 1024);
    xcb_get_property_reply_t* reply =
        xcb_get_property_reply(connection, request, nullptr);
    if (reply && reply->type != atom_type)
    {
        ADS_PRINT("ATOM TYPE MISMATCH (" << type << "). Expected: " << atom_type
                                         << "  but got " << reply->type);
        free(reply);
        return nullptr;
    }
...
//============================================================================
template<typename T>
void xcb_get_prop_list(WId window, const char* type, QVector<T>& ret,
                       unsigned int atom_type)
{
    xcb_get_property_reply_t* reply = _xcb_get_props(window, type, atom_type);
    if (reply && reply->format == 32 && reply->type == atom_type
        && reply->value_len > 0)
    {
        const xcb_atom_t* data =
            static_cast<const T*>(xcb_get_property_value(reply));
...
QString xcb_get_prop_string(WId window, const char* type)
{
    ...
    if (utf_atom != XCB_ATOM_NONE)
    {
        ...
        {
            const char* value =
                reinterpret_cast<const char*>(xcb_get_property_value(reply));
    ...
    xcb_get_property_reply_t* reply =
        _xcb_get_props(window, type, XCB_ATOM_STRING);
...
void xcb_add_prop(bool state, WId window, const char* type, const char* prop)
{
    ...
    xcb_connection_t* connection = x11_connection();
    xcb_change_property(connection, XCB_PROP_MODE_REPLACE, window, type_atom,
                        XCB_ATOM_ATOM, 32, atoms.count(), atoms.constData());
    xcb_flush(connection);
}
...
QString detectWindowManagerX11()
{
    ...
    // See:
    // https://specifications.freedesktop.org/wm-spec/1.3/ar01s03.html#idm46018259946000
    if (!is_platform_x11())
    {
        return "UNKNOWN";
    }
    xcb_connection_t* connection = x11_connection();
    xcb_screen_t* first_screen =
    ...
    xcb_get_prop_list(root, "_NET_SUPPORTING_WM_CHECK", sup_windows,
                      XCB_ATOM_WINDOW);
    if (sup_windows.length() == 0)
    {
        // This doesn't seem to be in use anymore, but wmctrl does the same so
        // lets play safe. Both XCB_ATOM_CARDINAL and XCB_ATOM_WINDOW break down
        // to a uint32_t, so reusing sup_windows should be fine.
        xcb_get_prop_list(root, "_WIN_SUPPORTING_WM_CHECK", sup_windows,
                          XCB_ATOM_CARDINAL);
    }
    if (sup_windows.length() == 0)
    {
                   ...
                   ads::eIcon CustomIconId)
{
    ...
    QPixmap normalPixmap =
        Button->style()->standardPixmap(StandarPixmap, 0, Button);
    Icon.addPixmap(internal::createTransparentPixmap(normalPixmap, 0.25),
                   QIcon::Disabled);
...
void repolishStyle(QWidget* w, eRepolishChildOptions Options)
{
    ...
    QList<QWidget*> Children =
        w->findChildren<QWidget*>(QString(), (RepolishDirectChildren == Options) ?
