...
struct AutoHideTabPrivate
{
                                    ...
                                    SideBarLocation Location, QMenu* Menu)
    {
        ...
        QObject::connect(Action, &QAction::triggered, _this,
                         &CAutoHideTab::onAutoHideToActionClicked);
...
{
    bool IconOnly = CDockManager::testAutoHideConfigFlag(
        CDockManager::AutoHideSideBarsIconOnly);
    ...
    else
    {
        ...
        _this->setOrientation((area == SideBarBottom || area == SideBarTop) ?
                                  Qt::Horizontal :
                                  Qt::Vertical);
    }
}
...
bool AutoHideTabPrivate::startFloating(eDragState DraggingState)
{
    ...
    case SideBarTop: StartPos.ry() = AutoHideContainer->rect().top() + 10; break;

    case SideBarBottom:
        StartPos.ry() = AutoHideContainer->rect().bottom() - 10;
        break;

    case SideBarNone: return false;
    }
...
bool CAutoHideTab::event(QEvent* event)
{
    ...
    {
    case QEvent::Enter:
    case QEvent::Leave: d->forwardEventToDockContainer(event); break;
...
void CAutoHideTab::contextMenuEvent(QContextMenuEvent* ev)
{
    ...
    auto IsPinnable =
        d->DockWidget->features().testFlag(CDockWidget::DockWidgetPinnable);
    ...
    if (ev->button() == Qt::LeftButton)
    {
        ...
        case DraggingFloatingWidget:
            ...
            if (d->DockWidget->autoHideDockContainer()
                && d->DragStartOrientation != orientation())
            {
                d->DockWidget->autoHideDockContainer()
