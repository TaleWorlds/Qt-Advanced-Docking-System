...
struct AutoHideSideBarPrivate
{
     ...
     */
    bool isHorizontal() const { return Qt::Horizontal == Orientation; }
...
//============================================================================
CAutoHideSideBar::CAutoHideSideBar(CDockContainerWidget* parent,
    ...
    : Super(parent), d(new AutoHideSideBarPrivate(this))
{
    ...
    d->TabsLayout = new QBoxLayout(d->Orientation == Qt::Vertical ?
                                       QBoxLayout::TopToBottom :
                                       QBoxLayout::LeftToRight);
	if (d->Orientation == Qt::Horizontal)
	{
		d->TabsLayout->setContentsMargins(4, 0, 4, 0);
	}
	else
	{
		d->TabsLayout->setContentsMargins(0, 4, 0, 4);
	}
    d->TabsLayout->setSpacing(4);
...
CAutoHideSideBar::~CAutoHideSideBar()
{
    ...
    auto Tabs =
        findChildren<CAutoHideTab*>(QString(), Qt::FindDirectChildrenOnly);
    for (auto Tab : Tabs)
    {
...
//============================================================================
CAutoHideDockContainer* CAutoHideSideBar::insertDockWidget(
    int Index, CDockWidget* DockWidget)
{
    auto AutoHideContainer = new CAutoHideDockContainer(
        DockWidget, d->SideTabArea, d->ContainerWidget);
    DockWidget->dockManager()->dockFocusController()->clearDockWidgetFocus(
        DockWidget);
                                         ...
                                         int TabIndex)
{
    ...
    if (SideBar == this)
    {
        ...
        if (AutoHideWidget->tabIndex() == TabIndex
            || (AutoHideWidget->tabIndex() + 1) == TabIndex)
        {
            return;
        }
...
bool CAutoHideSideBar::eventFilter(QObject* watched, QEvent* event)
{
    ...
    switch (event->type())
    {
    case QEvent::ShowToParent: show(); break;
