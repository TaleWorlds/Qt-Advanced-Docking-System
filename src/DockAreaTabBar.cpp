...
CDockAreaTabBar::CDockAreaTabBar(CDockAreaWidget* parent)
    : QScrollArea(parent), d(new DockAreaTabBarPrivate(this))
{
    d->DockArea = parent;
...
CDockWidgetTab* CDockAreaTabBar::currentTab() const
{
    ...
    {
        return qobject_cast<CDockWidgetTab*>(
            d->TabsLayout->itemAt(d->CurrentIndex)->widget());
    }
}
...
bool CDockAreaTabBar::eventFilter(QObject* watched, QEvent* event)
{
    ...
    case QEvent::LayoutRequest: updateGeometry(); break;
