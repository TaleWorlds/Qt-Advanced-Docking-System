...
struct DockWidgetPrivate
{
     ...
     * This prevents the auto hide dock widgets from being pinned to an empty dock
     * area
     */
...
//============================================================================
DockWidgetPrivate::DockWidgetPrivate(CDockWidget* _public) : _this(_public)
{}
...
void DockWidgetPrivate::showDockWidget()
{
    if (!Widget)
    {
        ...
        {
            Q_ASSERT(
                !Features.testFlag(CDockWidget::DeleteContentOnClose)
    ...
    else
    {
        ...
        // the auto hide widget by moving it's contents to parent container While
        // restoring state, opened dock widgets are not valid
        if (Container->openedDockWidgets().count() == 0 && DockArea->isAutoHide()
...
void DockWidgetPrivate::setupToolBar()
{
    ...
    _this->connect(_this, SIGNAL(topLevelChanged(bool)),
                   SLOT(setToolbarFloatingStyle(bool)));
}
...
void DockWidgetPrivate::setToolBarStyleFromDockManager()
{
    ...
    _this->setToolBarIconSize(DockManager->dockWidgetToolBarIconSize(State),
                              State);
...
//============================================================================
void CDockWidget::setWidgetFactory(FactoryFunc createWidget,
                                   eInsertMode insertMode)
{
    if (d->Factory)
    {
        delete d->Factory;
...
{
    return isAutoHide() ? autoHideDockContainer()->sideBarLocation() :
                          SideBarNone;
}
...
void CDockWidget::toggleViewInternal(bool Open)
{
    ...
    CDockWidget* TopLevelDockWidgetBefore =
        DockContainer ? DockContainer->topLevelDockWidget() : nullptr;
    ...
    CDockWidget* TopLevelDockWidgetAfter =
        DockContainer ? DockContainer->topLevelDockWidget() : nullptr;
    CDockWidget::emitTopLevelEventForWidget(TopLevelDockWidgetAfter, true);
    CFloatingDockContainer* FloatingContainer =
        DockContainer ? DockContainer->floatingWidget() : nullptr;
...
bool CDockWidget::event(QEvent* e)
{
    switch (e->type())
    {
    case QEvent::Hide: Q_EMIT visibilityChanged(false); break;

    case QEvent::Show:
        Q_EMIT visibilityChanged(geometry().right() >= 0
                                 && geometry().bottom() >= 0);
        break;
...
void CDockWidget::setToolBar(QToolBar* ToolBar)
{
    ...
    this->connect(this, SIGNAL(topLevelChanged(bool)),
                  SLOT(setToolbarFloatingStyle(bool)));
...
void CDockWidget::setToolbarFloatingStyle(bool Floating)
{
    ...
    auto IconSize = Floating ? d->ToolBarIconSizeFloating :
                               d->ToolBarIconSizeDocked;
...
bool CDockWidget::closeDockWidgetInternal(bool ForceClose)
{
    ...
    if (features().testFlag(CDockWidget::DockWidgetDeleteOnClose))
    {
        ...
        {
            CFloatingDockContainer* FloatingWidget =
                internal::findParent<CFloatingDockContainer*>(this);
...
void CDockWidget::setAutoHide(bool Enable, SideBarLocation Location, int TabIndex)
{
    ...
    {
        auto area = (SideBarNone == Location) ?
                        DockArea->calculateSideTabBarArea() :
                        Location;
