...
struct DockWidgetTabPrivate
{
    ...
    void updateCloseButtonSizePolicy()
    {
        ...
        SizePolicy.setRetainSizeWhenHidden(
            Features.testFlag(CDockWidget::DockWidgetClosable)
    ...
    void updateIcon()
    {
        ...
        {
            IconLabel->setPixmap(Icon.pixmap(_this->style()->pixelMetric(
                QStyle::PM_SmallIconSize, nullptr, _this)));
        }
                                    ...
                                    SideBarLocation Location, QMenu* Menu)
    {
        ...
        QObject::connect(Action, &QAction::triggered, _this,
                         &CDockWidgetTab::onAutoHideToActionClicked);
        return Action;
    }
};
// struct DockWidgetTabPrivate
...
void DockWidgetTabPrivate::createLayout()
{
    ...
    _this->connect(TitleLabel, SIGNAL(elidedChanged(bool)),
...
bool DockWidgetTabPrivate::startFloating(eDragState DraggingState)
{
    ...
    if (DraggingFloatingWidget == DraggingState)
    {
        ...
        {
            Overlay = DockWidget->floatingDockContainer()->containerOverlay();
        }
...
void CDockWidgetTab::mouseReleaseEvent(QMouseEvent* ev)
{
    ...
    else if (ev->button() == Qt::MiddleButton)
    {
        if (CDockManager::testConfigFlag(CDockManager::MiddleMouseButtonClosesTab)
            && d->DockWidget->features().testFlag(
...
void CDockWidgetTab::mouseMoveEvent(QMouseEvent* ev)
{
    ...
    int DragDistanceY = qAbs(d->GlobalDragStartMousePosition.y()
    ...
    if (DragDistanceY >= CDockManager::startDragDistance() || MouseOutsideBar)
    {
        ...
        return;
             ...
             && (internal::globalPositionOf(ev) - d->GlobalDragStartMousePosition)
                        .manhattanLength()
                    >= QApplication::startDragDistance())  // Wait a few pixels
                                                           // before start moving
    {
...
void CDockWidgetTab::contextMenuEvent(QContextMenuEvent* ev)
{
    ...
    const bool isFloatable =
        d->DockWidget->features().testFlag(CDockWidget::DockWidgetFloatable);
    const bool isNotOnlyTabInContainer =
        !d->DockArea->dockContainer()->hasTopLevelDockWidget();
    ...
    if (!isTopLevelArea)
    {
                ...
                CDockManager::AutoHideFeatureEnabled))
        {
            ...
            auto IsPinnable = d->DockWidget->features().testFlag(
...
void CDockWidgetTab::setActiveTab(bool active)
{
    ...
    if (CDockManager::testConfigFlag(CDockManager::ShowTabTextOnlyForActiveTab)
        && !d->Icon.isNull())
    {
        if (active)
...
void CDockWidgetTab::setIcon(const QIcon& Icon)
{
    ...
    if (!d->IconLabel)
    {
        ...
        Layout->insertSpacing(
            1, qRound(1.5 * Layout->contentsMargins().left() / 2.0));
    }
    else if (Icon.isNull())
    {
