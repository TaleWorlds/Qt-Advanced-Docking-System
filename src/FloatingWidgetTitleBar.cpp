...
void FloatingWidgetTitleBarPrivate::createLayout()
{
    ...
    _this->connect(MaximizeButton, &QPushButton::clicked, _this,
...
CFloatingWidgetTitleBar::CFloatingWidgetTitleBar(CFloatingDockContainer* parent)
    : QFrame(parent), d(new FloatingWidgetTitleBarPrivate(this))
{
    ...
    else if (CDockManager::testConfigFlag(
    ...
    if (CDockManager::testConfigFlag(
            CDockManager::FloatingContainerForceQWidgetTitleBar))
    {
        auto normalPixmap = this->style()->standardPixmap(
            QStyle::StandardPixmap::SP_TitleBarNormalButton, 0,
            d->MaximizeButton);
        d->NormalIcon.addPixmap(normalPixmap, QIcon::Normal);
        d->NormalIcon.addPixmap(
            internal::createTransparentPixmap(normalPixmap, 0.25),
            QIcon::Disabled);

        auto maxPixmap = this->style()->standardPixmap(
            QStyle::SP_TitleBarMaxButton, 0, d->MaximizeButton);
        d->MaximizeIcon.addPixmap(maxPixmap, QIcon::Normal);
        d->MaximizeIcon.addPixmap(
            internal::createTransparentPixmap(maxPixmap, 0.25), QIcon::Disabled);
