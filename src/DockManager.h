...
protected:
    ...
    /**
     * Restore floating widgets hidden by an earlier call to
...
public:
	...
	enum eConfigFlag
	{
		...
		FloatingContainerHasWidgetTitle =
			0x40000, //!< If set, the Floating Widget window title reflects the title of the current dock widget otherwise it
					 //!< displays the title set with `CDockManager::setFloatingContainersTitle` or application name as window
					 //!< title
		...
		HideSingleCentralWidgetTitleBar =
			0x100000, //!< If there is only one single visible dock widget in the main dock container (the dock manager) and if
					  //!< this flag is set, then the titlebar of this dock widget will be hidden
		//!< this only makes sense for non draggable and non floatable widgets and enables the creation of some kind of
		//!< "central" widget
        ...
        DefaultBaseConfig = DefaultDockAreaButtons | ActiveTabHasCloseButton
                            ...
                            | FloatingContainerHasWidgetTitle,  ///< default base
                                                                ///< configuration
                                                                ///< settings
        ...
        DefaultNonOpaqueConfig =
            DefaultBaseConfig | DragPreviewShowsContentPixmap,  ///< the
                                                                ///< default
                                                                ///< configuration
                                                                ///< for non
                                                                ///< opaque
     ...
     * \return Returns the dock area widget that contains the new DockWidget
     */
    CDockAreaWidget* addDockWidgetToContainer(
        DockWidgetArea area, CDockWidget* Dockwidget,
     ...
     */
    CAutoHideDockContainer* addAutoHideDockWidget(SideBarLocation Location,
     ...
     * \return Returns the CAutoHideDockContainer that contains the new DockWidget
     */
    CAutoHideDockContainer* addAutoHideDockWidgetToContainer(
        SideBarLocation Location, CDockWidget* Dockwidget,
        CDockContainerWidget* DockContainerWidget);
     ...
     * any floating widget
     */
    unsigned int zOrderWidgetIndex() const override;
	...
	bool checkFormat(const QByteArray& state, int version = 0);

	/**
	 * Returns a copy of the perspectives registered in the dock manager,
	 * and the corresponding state with them
	 */
	QMap<QString, QByteArray> perspectives() const;
     ...
     * widget. If central widget is set, it will be the only dock widget that will
     * resize with the dock container. A central widget if not movable, floatable
     * or closable and the titlebar of the central dock area is not visible. If
     * the given widget could be set as central widget, the function returns the
     * created dock area. If the widget could not be set, because there is already
     * a central widget, this function returns a nullptr. To clear the central
     * widget, pass a nullptr to the function. \note Setting a central widget is
     * only possible if no other dock widgets have been registered before. That
     * means, this function should be the first function that you call before you
     * add other dock widgets. \retval != 0 The dock area that contains the
     * central widget \retval nullptr Indicates that the given widget can not be
     * set as central widget because there is already a central widget.
     */
    ...
    void removeToggleViewActionFromMenu(QAction* ToggleViewAction,
                                        const QString& Group = QString());
     ...
     * effect.
     */
    void setSplitterSizes(CDockAreaWidget* ContainedArea,
     ...
     * might be better.
     */
    void setDockWidgetToolBarStyle(Qt::ToolButtonStyle Style,
                                   CDockWidget::eState State);
     ...
     * might be better.
     */
    void setDockWidgetToolBarIconSize(const QSize& IconSize,
                                      CDockWidget::eState State);
     ...
     * \code
     */
    void lockDockWidgetFeaturesGlobally(CDockWidget::DockWidgetFeatures Features =
                                            CDockWidget::GloballyLockableFeatures);
...
public Q_SLOTS:
    ...
    /**
     * hide CDockManager and all floating widgets (See Issue #380). Calling
     * regular QWidget::hide() hides the CDockManager but not the floating
     * widgets;
     */
